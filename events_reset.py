...
# All the Files
file_events = open('/home/gapp/turntree.github.io/CryptoData/events.csv', 'w')

# All the headers
header_events = ['id', 'coin_id', 'coin_name', 'coin_symbol', 'date_event', 'created_date', 'title', 'description',
                 'category_id', 'category_name', 'proof', 'source', 'is_hot', 'vote_count', 'positive_vote_count',
                 'percentage', 'can_occur_before', 'load_date']
...
while i < 100:
    i = i + 1
    try:
        eventApi = 'https://api.coinmarketcal.com/v1/events?access_token=' + access_token + '&page=' + str(
            i) + '&max=200&dateRangeStart=01%2F01%2F2010'
        ...
        for x in data:
            try:
                ...
                created_date = datetime.datetime.strptime(
                    x["created_date"].split('T', 1)[0] + ' ' + x["created_date"].split('T', 1)[1].split('+', 1)[0],
                    '%Y-%m-%d %H:%M:%S')
                ...
                for y in coins:
                    ...
                    for z in categories:
                        ...
                        row_event = [id, coin_id, coin_name, coin_symbol, date_event, created_date, title, description,
                                     category_id, category_name, proof, source, is_hot, vote_count, positive_vote_count,
                                     percentage, can_occur_before, sysdate]
                        file_events.write(','.join(str(e).encode("utf8") for e in row_event) + '\n')
            except Exception as e:
                print(x)
                print("for ith loop, failed for id", i, id)
                print(e)
    except Exception as e:
